...
export class SendWorkPermitModalComponent implements OnInit {
  ...
  onExtendWork() {
    let params = {
      ...
      WorkPermitNo: this.selectItem.Item.WorkPermitNo
    };

    this.service.ExtendWork(params).subscribe(response => {
      if (response.Code == 200) {
        this.toastService.success(this.Message.WorkPermit.ExtendSuccess);

        this.selectItem.Item.StatusText = 'Open';
        this.selectItem.Item.Status = '1';
        this.selectItem.Item.CreateSentDate = '';
        this.selectItem.Item.ProjectSentApprovalDate = '';
        this.selectItem.Item.ZoneSentApprovalDate = '';
        this.selectItem.Item.SafeSentApprovalDate = '';
      }
      else {
        this.toastService.error(response.Data.ErrorMessage);
      }
    });
  }
